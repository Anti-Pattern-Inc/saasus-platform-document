...
Add an `import` statement and `app.use("/callback", callbackRouter)` to `api/app.ts`, placing it immediately after `app.use("/login", loginRouter);` and before the 404 handler.

:::caution
In Express, middleware and routes are processed in the order they are registered. If a 404 handler is registered first, the `/callback` request will be caught by it and will never reach the callback router, causing a login error.
:::
