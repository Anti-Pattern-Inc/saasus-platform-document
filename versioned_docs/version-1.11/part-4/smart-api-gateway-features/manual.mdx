...
In the implementation example, the inventory management API uses environment ID "3" (production environment) and specific purchaser ID (user ID) to filter inventory data according to environment and user.
...
The following are examples of API access utilizing tenant ID, environment ID, and user ID through SaaSus Identity integration:

**Get all inventory (filtering by tenant and environment):**
```sh
...
In this example, inventory data is filtered based on environment ID "3" (production environment) and specific user ID (purchaserId).
