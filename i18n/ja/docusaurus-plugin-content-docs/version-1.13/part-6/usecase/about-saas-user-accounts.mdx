...
<a href="/ja/docs/part-6/usecase/about-saas-user-accounts/create-tenant" target="_blank">テナントの作成</a>を参考に、テナント情報が登録できるよう修正してみましょう。
...
テナント情報が取得でき、テナントユーザーが作成できたら、通常のログインフローでログインができるか試してみましょう。
...
### ３．３．SaaS 運用コンソールからアカウント作成するケースの実装
