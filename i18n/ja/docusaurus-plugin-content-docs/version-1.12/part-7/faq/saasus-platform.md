...
A. はい、可能です。本番ドメインの設定（検証）が完了していれば、コールバック先をローカル環境の URL に指定することで動作を確認いただけます。
 ...
 - **コールバック URL の追加設定**  
[基本設定] ＞ [認証後遷移先] 画面にて、登録されているドメインのステータスが **「有効（検証済み）」**、 **「独自ドメインによる認証メールの配信が有効になっています。」** になっているかご確認ください。  
...
1. **「テナント招待機能」を利用する方法**  
[テナント招待機能](https://docs.saasus.io/ja/docs/reference/auth-api#tag/invitation)を利用してユーザーを招待することが可能です。テナント招待機能は招待中のユーザーの状態を管理する機能も提供しております。招待メール内のリンクからユーザー自身がパスワードを設定できるフローが構築できます。
...
2. ユーザー作成APIを利用する方法  
[CreateUser API](https://docs.saasus.io/ja/docs/reference/auth-api#tag/saasUser/operation/CreateSaasUser) を呼び出す際、パスワードを**未設定（空）**の状態でリクエストしてください。
...
ログイン完了後、自動的にパスワード変更画面が表示され、任意のパスワードへの変更が強制される仕組みになっています。
