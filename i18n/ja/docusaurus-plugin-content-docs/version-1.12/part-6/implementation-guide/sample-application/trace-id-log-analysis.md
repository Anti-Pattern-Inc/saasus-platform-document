...
```mermaid
flowchart LR
    FE["フロントエンド<br/>UUID 生成"] -->|X-SaaSus-Trace-Id| BE["バックエンド<br/>context に格納"]
    BE -->|X-SaaSus-Trace-Id| API["SaaSus API<br/>ログに記録"]
```
...
Trace ID が伝播されると、SaaSus Platform 側の API ログに `trace_id` フィールドとして記録されます。

### SaaS 開発コンソールでの確認

SaaS 開発コンソールの「APIログ」から、`trace_id` でフィルタリングすることで関連する API リクエストを一覧表示できます。
...
| ログ分析 | SaaS 開発コンソールの APIログ、または API で `trace_id` によるフィルタリング |
