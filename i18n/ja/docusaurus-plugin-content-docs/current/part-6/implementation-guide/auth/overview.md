...
title: "認証API実装ガイド"
slug: "overview"
excerpt: "SaaSus Platform 認証APIを使った独自ログイン画面の実装ガイド概要"
hidden: false
...
SaaSus Platform 認証APIを使って、独自のログイン画面からログイン処理を実装するための概要を説明します。

:::info
基本実装の具体的なコードは [認証APIを利用した基本実装](/docs/part-6/implementation-guide/auth/basic-sign-in) を、応用例は [認証APIを利用した応用例（IDログイン）](/docs/part-6/implementation-guide/auth/advanced-sign-in) を、セキュリティ実装は [認証セキュリティ・ポリシーの実装](/docs/part-6/implementation-guide/auth/security) をご参照ください。
:::
...
本ドキュメントでは、この認証APIを使って「独自ログイン画面」を使った、ログイン処理を実装する方法を説明します。
...
本実装ガイドのサンプルアプリケーションは、以下のリポジトリで公開しています。
...
![ログイン画面](/img/part-6/implementation-guide/auth/overview/login-screen.png)
- **新パスワード設定画面**: 初回ログイン時のパスワード変更
- **認証API**: SaaSus Platform Auth API との SRP 認証フロー
- **IDログイン**: テナント属性 `login_domain` を活用したユーザー名＋テナントIDによるログイン
- **トークン管理**: HttpOnly Cookie による安全なトークン保存
- **ログアウト処理**: Cookie クリアによるセッション終了

具体的な実装内容は、[認証APIを利用した基本実装](/docs/part-6/implementation-guide/auth/basic-sign-in) をご覧ください。
