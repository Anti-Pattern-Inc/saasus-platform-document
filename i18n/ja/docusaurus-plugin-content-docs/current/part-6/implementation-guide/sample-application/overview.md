...
- **認証方式選択**: 認証アプリ（TOTP）とメール認証から選択
- **MFA状態確認**: 有効/無効と現在の認証方式の確認
- **認証アプリ設定**: QRコード表示と6桁コードの検証
- **メール認証設定**: ワンクリックでメール認証を有効化
- **方式切り替え**: 設定済みの認証方式から別の方式へ変更
- **MFA無効化**: 多要素認証の解除
...
1. SaaS 開発コンソールで認証後遷移先URLを設定（`http://localhost:3000/callback`）
2. 必要な設定値を取得：
   - ログイン画面のURL
   - APIキー等の認証情報
