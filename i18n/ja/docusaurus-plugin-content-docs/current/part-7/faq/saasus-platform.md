...
updatedAt: "Sun Feb 09 2026 10:19:52 GMT+0000 (Coordinated Universal Time)"
---
...
**Q: SaaSusのユーザー数カウントの考え方について教えてください**

A. SaaSusでは、お客様のSaaSアプリケーションを利用する一般ユーザー（エンドユーザー）がカウント対象となります。SaaSus開発コンソール（管理画面）に登録されている管理者アカウント等は含まれません。

ただし、以下の通りプランによってカウントの定義が異なります。

| プラン | カウントの定義 | 対象となる数値 |
| :--- | :--- | :--- |
| **スターター** | **月間アクティブ ユーザー** | 実際にログインしたユーザーのみを「従量課金」の対象としてカウント |
| **スタンダード 以上** | **登録ユーザー数** | ログインの有無に関わらず、登録されている全ユーザー数を「プラン上限値」の対象としてカウント |

**■ 注意点**
スタンダードプラン以上では、「実際に利用したか」ではなく、「アカウントを保持している人数」が契約プランの制限枠（上限）に充当されます。