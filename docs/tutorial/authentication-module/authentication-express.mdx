...
The redirect destination URL is now received by the SaaSus SDK standard Callback process ([http://localhost:3000/callback](http://localhost:3000/callback)), and in that process, the browser's Local Storage or remember authentication information in a cookie.
