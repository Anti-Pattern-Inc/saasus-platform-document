...
AWS has never had a case where all AZs in a region stopped simultaneously, so we consider the risk low.  
...
**Q. What authentication methods are available for multi-factor authentication (MFA)?**  
A. SaaSus Platform supports the following two methods for multi-factor authentication (MFA). You can configure either method on a per-user basis.

 - **Authenticator App (TOTP)**  
Uses an authenticator app such as Google Authenticator, and performs the second-factor authentication with the one-time code displayed in the app. Setup is easy by scanning a QR code.

 - **Email**  
At login, an authentication code is sent to the registered email address to perform the second-factor authentication.

Note that using multi-factor authentication (MFA) requires implementation on the SaaS application side. For details on how to use the API and implementation examples, see [Implementing Multi-Factor Authentication](/docs/part-6/implementation-guide/sample-application/multi-factor-authentication).

---

**Q. Can users who use email as their multi-factor authentication (MFA) method reset their password from the login screen?**  
A. No. Users who use "Email" as their multi-factor authentication (MFA) method cannot reset their password from the login screen generated by SaaSus Platform.  
To reset the password for such a user, an administrator should perform the password reset from the User Management screen in the SaaS Operation Console. For details, see [(Old Screen) Search, Edit, Create, or Assign Users to Tenants](/docs/part-5/user-management/user-crud).
...
If not verified, you need to correctly set the displayed DNS records in your DNS server (such as Amazon Route 53) and complete the verification.
...
This allows you to improve convenience during development and use while maintaining a secure state.

 - **Implementation Method Reference**  
For specific procedures on using refresh tokens, please refer to [Updating Authentication Information Using Refresh Tokens](https://docs.saasus.io/docs/part-6/usecase/authentication-methods#refresh-token) in the official documentation.
...
A. SaaSus Platform allows you to check the current metering values for each tenant from the Stripe Dashboard or API. We are also currently planning to provide a dashboard function that visualizes detailed usage trends.

 - **Checking Current Usage Status**  
The latest metering values for each tenant can be referenced through the SaaSus SDK/API.

 - **Checking via Stripe Dashboard**  
If you are using Stripe integration, you can open the target customer (tenant) from the "Customers" menu in the Stripe Dashboard to check the linked subscriptions, issued invoices, and usage-based metering amounts.
...
SaaSus Platform's authentication API adopts SRP (Secure Remote Password) authentication.
With SRP authentication, the password itself is not sent to the authentication platform; only verification values derived from the password are exchanged to perform authentication. By leveraging this mechanism, even when building a custom login screen, you can achieve free styling with HTML/CSS while maintaining security.
...
To freely display available hours, maintenance information, etc., you need to implement your own login screen using the authentication API provided by SaaSus Platform.
...
Even when performing custom implementation, SaaSus Platform's authentication API adopts SRP (Secure Remote Password) authentication, so the password itself is not sent to the authentication platform during authentication, making it possible to enhance operational convenience.
...
A. Currently, we do not provide a function for customers to directly set "Geographic Restrictions" on the login screen (Amazon CloudFront) provided as standard by SaaSus Platform.

 - **Method to Achieve Japan-Only Access**  
If you want to limit access origins to within Japan, we recommend a configuration where you build and distribute the login screen via your own environment (such as AWS, Google Cloud, or Azure) using the authentication API of SaaSus Platform.
...
**Q. Can I switch back (downgrade) from a paid plan to the Free plan?**

A. Under the current specifications, you cannot directly switch back (downgrade) from a paid plan to the Free plan.  
To switch to the Free plan, you need to follow the steps below.

1. **Cancel your current paid plan**
2. **Create a new account (tenant) and select the Free plan**

 - **About migrating your settings**  
Along with the steps above, you need to extract the configuration information from your current account and migrate (reconfigure) it to the new account.  
When retrieving your configuration information, using the [MCP Server feature](/docs/part-4/mcp-server/setup) makes the process smoother.

---

**Q. How do I upgrade from the Free plan to a paid plan?**

A. You can upgrade (apply for a paid plan) from "Account Settings > Pricing Plans" in the SaaS Development Console.  
Unlike a downgrade, you can complete the procedure directly from the management console.

 - **About billing after applying**  
Billing is on a monthly basis starting from the application date, invoices are sent via Stripe, and the plan renews automatically each month.

 - **Applying via AWS Marketplace**  
You can also apply via AWS Marketplace.

 - **Plan pricing and limits**  
For the pricing and limits of each plan, such as the number of environments, users, and tenants, please refer to [Procedure for Applying for a Paid Plan](/docs/part-4/account-settings/procedure-for-applying-for-a-paid-plan).
